...
<script type="text/javascript">
  $(function() { // 자동 실행
    ...
    $('#content', frm_reply).on('click', check_login);  // 댓글 작성시 로그인 여부 확인 event
  ...
  function check_login() {
    var frm = $('#frm_reply');  // frm_reply (form reply를 찾아서)
    if ($('#memberno', frm_reply).val().length == 0) {     // val = validation, 로그인을 하지 않은 경우
  ...
  function reply_create() {
    var frm_reply = $('#frm_reply') 
    if (check_login() != false) { // 로그인 한 경우만
        var params = frm_reply.serialize();    // form에 입력된 데이터를 가져와
        //alert(params);                             // 체크해본다 확인이 되면 alert, return; 주석처리
        //return;
        ...
        $.ajax({
          ...
          success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
            ...
            if (rdata.cnt > 0) {
              ...
              list_by_contentsno_join(); // 댓글 목록을 새로 읽어온다
    ...
    $.ajax({
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        for (i=0; i < rdata.list.length; i++) {
          ...
          msg += "  " + row.rdate;
          if ('${sessionScope.memberno}' == row.memberno) { // 글쓴이 일치여부 확인, 본인의 글만 삭제 가능★
