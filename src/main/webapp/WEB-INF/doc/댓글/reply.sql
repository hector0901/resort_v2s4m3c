...
VALUES((SELECT NVL(MAX(replyno), 0) + 1 as replyno FROM reply),
             45, 1, '댓글1', '1234', sysdate);
INSERT INTO reply(replyno, contentsno, memberno, content, passwd, rdate)
VALUES((SELECT NVL(MAX(replyno), 0) + 1 as replyno FROM reply),
             45, 1, '댓글2', '1234', sysdate);
INSERT INTO reply(replyno, contentsno, memberno, content, passwd, rdate)
VALUES((SELECT NVL(MAX(replyno), 0) + 1 as replyno FROM reply),
             45, 1, '댓글3', '1234', sysdate);             
...
ORDER BY replyno DESC;
 ...
 ------- ---------- -------- ------- ------ ---------------------
       3          45        1 댓글3     1234   2019-12-17 16:59:38.0
       2          45        1 댓글2     1234   2019-12-17 16:59:37.0
       1          45        1 댓글1     1234   2019-12-17 16:59:36.0
...
7-2) reply + member join 목록 + 회원 ID
SELECT m.id,
           r.replyno, r.contentsno, r.memberno, r.content, r.passwd, r.rdate
FROM member m,  reply r
...
7-3) 더보기 버튼 페이징, contentsno 별 목록
