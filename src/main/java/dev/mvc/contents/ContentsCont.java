...
public class ContentsCont {
 ...
 public ModelAndView read(int contentsno) {
   ...
   mav.setViewName("/contents/read_img_attachfile_reply_add_pg"); // /webapp/contents/read_img_attachfile_reply_add.jsp
   return mav;
 }
...
//  }
  ...
  /**
   * 로그인 여부만 체크함으로 다른 사용자의 글을 삭제 할 수 있는 화면이 출력됨. 삭제 폼, attachfile Ajax 기반 삭제 지원
   * 
   * @return
   */
  @RequestMapping(value = "/contents/delete.do", method = RequestMethod.GET)
  ...
  public ModelAndView img_delete(HttpServletRequest request, int contentsno, int cateno, String passwd,
   ...
   /**
   * 메인 이미지 수정 처리 http://localhost:9090/resort/contents/img_update.do 기존 이미지 삭제후
   * 새로운 이미지 등록(수정 처리)
   * 
   * @return
   */
